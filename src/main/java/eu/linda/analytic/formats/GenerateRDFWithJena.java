...
public class GenerateRDFWithJena {
    ...
    public Model generateRDFModel(Analytics analytics, AbstractList dataToExport) {
        ...
        String datasetContextToString = "analytics" + analytics.getId()+ "V" + 1 + "Date" + today;

   
        
        // Define some triplets to convert.
        Object[][] triplets = {
            {0.57, "attributeValue", "jumped", "over the lazy dog.", "http://health.data.ny.gov/resource/hbu9-xsrx/", "http://health.data.ny.gov/resource/hbu9-xsrx/1", 111, 6},
        ...
        Resource analytic_process_statement = model.createResource(NS + analytic_process_info[1]);
        analytic_process_statement.addProperty(OWL.versionInfo, "1.0.0");
        analytic_process_statement.addLiteral(analyzedField, analytic_process_info[5]);
        ...
        for (Object[] triplet : triplets) {
            ...
            Resource analytic_result_node_statement = model.createResource(NS + triplet[7] + "/" + triplet[6]);
            Resource analytic_input_node_statement = model.createResource((String) triplet[5]);
            ...
            analytic_result_node_statement.addProperty(predictedValue, triplet[0].toString());
